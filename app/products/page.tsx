...
export default function ProductListPage() {
  ...
  const filteredProducts = products.filter(product => {
    const matchesCategory = selectedCategory === null || product.category?.c_id === selectedCategory
    const matchesSearch =
      searchTerm === '' ||
      ...
      <div className="max-w-6xl mx-auto p-6">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {selectedCategory && (
                <span className="inline-flex items-center gap-1 px-3 py-1 bg-orange-100 text-orange-800 rounded-full text-sm">
                  <span className="text-xs">{getCategoryIcon(categories.find(cat => cat.c_id === selectedCategory)?.name)}</span>
                  {categories.find(cat => cat.c_id === selectedCategory)?.name}
                  <button
        ...
        <div className="mb-6">
          <p className="text-orange-700">
            ...
            {selectedCategory && ` ในหมวดหมู่ "${categories.find(cat => cat.c_id === selectedCategory)?.name}"`}
          </p>
        </div>
